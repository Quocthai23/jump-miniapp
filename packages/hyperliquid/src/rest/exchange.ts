...
export class ExchangeAPI {
  ...
  private async checkAndSetReferralIfNeeded(): Promise<void> {
    ...
    try {
      ...
      if (!referralInfo.referredBy) {
        ...
        console.log(
          `[Hyperliquid SDK] Setting referral code: ${codeToUse} for user: ${userAddress}`
        );
        await this.setReferrer(codeToUse);
      } else {
        console.log(
          `[Hyperliquid SDK] User ${userAddress} already has referral: ${referralInfo.referredBy.code}`
        );
      }
    } catch (error) {
      ...
      try {
        ...
        if (userAddress) {
          const codeToUse = this.referralCode || CONSTANTS.JUMPMEME_CODE;
          console.log(
            `[Hyperliquid SDK] Referral check failed, setting referral code anyway: ${codeToUse}`
          );
          await this.setReferrer(codeToUse);
        }
      } catch (setError) {
  ...
  private async getAssetIndex(symbol: string): Promise<number> {
    ...
    const index = await this.symbolConversion.getAssetIndex(normalizedSymbol);
    if (index === undefined) {
      throw new Error(`Unknown asset: ${symbol} (normalized to ${normalizedSymbol})`);
    }
  ...
  async approveAgent(request: ApproveAgentRequest): Promise<any> {
    console.log('approveAgent called with request:', request);
