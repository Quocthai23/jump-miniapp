...
export class WebSocketPayloadManager {
  ...
  async marketClose(
    ...
    return this.customOperations.marketClose(address, symbol, size, px, slippage, cloid);
  }
   ...
   * This is a custom composite operation that uses native methods
   */
  async closeAllPositions(address: string, slippage: number = 0.05): Promise<any> {
    return this.customOperations.closeAllPositions(address, slippage);
  }
