...
interface ExtendedOpenOrder extends FrontendOpenOrders {
    children: any[];
    tif: string | null;
}
...
declare class ExchangeAPI {
    ...
    private disableAutoReferral?;
    private nonceCounter;
    private lastNonceTimestamp;
    constructor(testnet: boolean, privateKey: string, info: InfoAPI, rateLimiter: RateLimiter, symbolConversion: SymbolConversion, walletAddress: string | null | undefined, parent: Hyperliquid, vaultAddress?: string | null, referralCode?: string, disableAutoReferral?: boolean);
    private getVaultAddress;
    /**
     * Check if user has referral and set JUMPMEME code if not
     */
    private checkAndSetReferralIfNeeded;
    ...
    /**
     * Submit a pre-signed withdrawal action.
     *
     * This is useful in browser environments where the withdrawal needs to be
     * signed by an external wallet (e.g. MetaMask via wagmi) instead of the
     * SDK's internal ethers Wallet.
     *
     * The expected payload format matches the standard Hyperliquid withdraw3
     * request:
     *
     * {
     *   action: {
...
declare class WebSocketClient {
    ...
    private ensureWebSocketImplementation;
    isConnected(): boolean;
...
declare class CustomOperations {
    ...
    marketClose(address: string, symbol: string, size?: number, px?: number, slippage?: number, cloid?: string): Promise<OrderResponse>;
    closeAllPositions(address: string, slippage?: number): Promise<OrderResponse[]>;
}
...
declare class WebSocketPayloadManager {
     ...
     * This is a custom composite operation that uses native methods
     */
    marketClose(address: string, symbol: string, size?: number, px?: number, slippage?: number, cloid?: string): Promise<any>;
    /**
     * Close all positions via WebSocket POST
     * This is a custom composite operation that uses native methods
     */
    closeAllPositions(address: string, slippage?: number): Promise<any>;
    /**
...
declare const JUMPMEME_CODE = "JUMPMEME";
